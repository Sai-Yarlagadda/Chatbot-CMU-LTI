...
def txt_bulks(fileobj):
    ...
    for line in fileobj:
        if 'data' in line.lower() and 'event' in line.lower() and 'semester' in line.lower():
            if state == IN_PARAGRAPH:
                # Add the current paragraph to the buffer and check its length
                buffer += ''.join(paragraph)
                paragraph = []  # Reset current paragraph
    ...
    if paragraph:
        buffer += ''.join(paragraph)
    # If the final buffer meets the minimum length requirement, yield it
    if buffer:  # If there's content in the buffer but it doesn't meet the minimum length, still yield it
        yield buffer
